...
BRAINIAC_CKPT="${BRAINIAC_CKPT:-/mnt/disk2/hieupc2/Stroke_project/code/baseline_encoder/BrainIAC/src/checkpoints/BrainIAC_mock.ckpt}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/outputs}"
CUDA_DEVICE="${CUDA_DEVICE:-0}"

PREPROCESSED_FOLD="${PREPROCESSED_FOLD:-/mnt/disk2/hieupc2/Stroke_project/code/datasets/fold}"
RAW_FOLD="${RAW_FOLD:-/mnt/disk2/hieupc2/Stroke_project/code/datasets/fold_raw_trace}"
...
WANDB_PROJECT="${WANDB_PROJECT:-brainiac-soop-outcome}"
...
conda run -n hieupcvp env PYTHONPATH=. python "$SRC_DIR/train_lightning_soop_regression.py" \
...
run_experiment() {
  ...
  conda run -n hieupcvp env CUDA_VISIBLE_DEVICES="$CUDA_DEVICE" PYTHONPATH=. python "$SRC_DIR/train_lightning_soop_regression.py" \
    --config "$SRC_DIR/config_soop_regression.yml" \
    --fold-dir "$fold_dir" \
  ...
  local ckpt_path
  ckpt_path="$(ls -1 "$output_dir"/checkpoints/*.ckpt | head -n 1)"

  conda run -n hieupcvp env CUDA_VISIBLE_DEVICES="$CUDA_DEVICE" PYTHONPATH=. python "$SRC_DIR/eval_soop_regression.py" \
...
run_experiment "$PREPROCESSED_FOLD" "nihss" 1 "soop_nihss_image_tabular"
...
run_experiment "$RAW_FOLD" "nihss" 1 "soop_raw_nihss_image_tabular"

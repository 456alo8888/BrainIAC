...
class ViTBackboneNet(nn.Module):
    def __init__(self, simclr_ckpt_path):
        ...
        # MONAI 1.5.x registers cross-attention modules on TransformerBlock even
        # when with_cross_attention is false. BrainIAC.ckpt was saved without
        # those unused parameters, so remove them before strict backbone loading.
        for block in self.backbone.blocks:
            if hasattr(block, "with_cross_attention"):
                block.with_cross_attention = False
            if hasattr(block, "norm_cross_attn"):
                delattr(block, "norm_cross_attn")
            if hasattr(block, "cross_attn"):
                delattr(block, "cross_attn")
